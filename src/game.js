(function () {
    ...
    // Legacy helpers and VM mechanics tests retain an offscreen context. The game
    // canvas itself is exclusively WebGL; no 2D world is composited into the scene.
    var ctx = document.createElement('canvas').getContext('2d');
    var renderer3d = window.DK3D ? window.DK3D.create(canvas, CONTENT) : null;
    function screenMotion(dx,dy){return window.DK3D?window.DK3D.screenDirection(dx,dy):{x:dx,y:dy};}
    function screenAim(angle){return window.DK3D?window.DK3D.screenAngle(angle):angle;}
    function mouseWorld(){return renderer3d&&renderer3d.screenToWorld(mouse.x,mouse.y)||{x:mouse.x/currentZoom+camera.x,y:mouse.y/currentZoom+camera.y};}
    var TAU = Math.PI * 2;
...
var PALACE_ART = CONTENT.palaceArt || Object.create(null);
    ...
    function isWorldVisible(x,y,r){
        if(renderer3d&&!renderer3d.failed)return renderer3d.visible(x,y,r||0);
    ...
    function lineIntersectsView(x1,y1,x2,y2){if(renderer3d){var a=renderer3d.worldToScreen(x1,y1),b=renderer3d.worldToScreen(x2,y2);return !(Math.max(a.x,b.x)<-90||Math.min(a.x,b.x)>viewportWidth+90||Math.max(a.y,b.y)<-90||Math.min(a.y,b.y)>viewportHeight+90);}var left=camera.x-90,right=camera.x+viewportWidth/currentZoom+90,top=camera.y-90,bottom=camera.y+viewportHeight/currentZoom+90;if(Math.max(x1,x2)<left||Math.min(x1,x2)>right||Math.max(y1,y2)<top||Math.min(y1,y2)>bottom)return false;return true;}
    ...
    Player.prototype.update = function (step, now) {
        ...
        var worldMotion=screenMotion(dx,dy);dx=worldMotion.x;dy=worldMotion.y;
        ...
        if (aimJoystick.active && aimJoystick.strength>.12) {
            this.angle=screenAim(aimJoystick.angle);
            ...
            var aimPoint=mouseWorld();var worldMouseX=aimPoint.x;var worldMouseY=aimPoint.y;
...
var ENEMY_BASE = CONTENT.enemies;
    ...
    BiomeBoss.prototype.draw=function(){if(renderer3d){var vector=renderer3d.vectors,draw=(window.DK_BUILTIN_BOSS_VISUALS||{})[this.bossKey];if(vector&&draw){vector.begin();draw(this,Object.assign({},BOSS_BEHAVIOR_API,{ctx:vector}));vector.end();}return;}
    ...
    function renderWeaponPreview(canvasNode,w,large){if(window.DK3D&&canvasNode&&w){var pc=canvasNode.getContext('2d');if(!pc)return false;pc.clearRect(0,0,canvasNode.width,canvasNode.height);return window.DK3D.drawPreview(pc,'weapon',w,canvasNode.width/2,canvasNode.height/2,canvasNode.height*1.6,canvasNode.height*.95);}if(!canvasNode||!w||!canvasNode.getContext)return false;var preview=canvasNode.getContext('2d');if(!preview)return false;var oldCtx=ctx,width=canvasNode.width||240,height=canvasNode.height||90,scale=large?2.15:1.32;try{ctx=preview;ctx.setTransform(1,0,0,1,0,0);ctx.clearRect(0,0,width,height);ctx.fillStyle='rgba(0,0,0,.28)';ctx.fillRect(0,0,width,height);ctx.save();ctx.translate(width*.5-31*scale,height*.5);ctx.scale(scale,scale);drawHeldWeapon(w,true);ctx.restore();ctx.strokeStyle='rgba(255,255,255,.18)';ctx.globalAlpha=1;ctx.lineWidth=1;ctx.strokeRect(.5,.5,width-1,height-1);}finally{ctx=oldCtx;}return true;}
    ...
    function updateShopRoom(step,now,deltaMs) {
        ...
        var dx=0,dy=0;if(keys.w)dy-=1;if(keys.s)dy+=1;if(keys.a)dx-=1;if(keys.d)dx+=1;if(joystick.active){dx=joystick.x;dy=joystick.y;}var motion=screenMotion(dx,dy);dx=motion.x;dy=motion.y;if(dx||dy){var len=Math.hypot(dx,dy)||1,m=player.baseSpeed*(1+player.stats.speed);player.x+=dx/len*m*step;player.y+=dy/len*m*step;player.angle=Math.atan2(dy,dx);}player.x=clamp(player.x,shopCenter.x-820,shopCenter.x+820);player.y=clamp(player.y,shopCenter.y-440,shopCenter.y+660);
        if(!joystick.active&&!isTouchDevice()){var point=mouseWorld(),worldMouseX=point.x,worldMouseY=point.y;player.angle=Math.atan2(worldMouseY-player.y,worldMouseX-player.x);}
    ...
    function localInputState(){var dx=0,dy=0;if(keys.w)dy-=1;if(keys.s)dy+=1;if(keys.a)dx-=1;if(keys.d)dx+=1;if(joystick.active){dx=joystick.x;dy=joystick.y;}var motion=screenMotion(dx,dy);dx=motion.x;dy=motion.y;var aim=player?player.angle:0,aimTarget=null;if(aimJoystick.active&&aimJoystick.strength>.12)aim=screenAim(aimJoystick.angle);else if(player&&!joystick.active&&!isTouchDevice()){var point=mouseWorld(),worldMouseX=point.x,worldMouseY=point.y;aim=Math.atan2(worldMouseY-player.y,worldMouseX-player.x);}else if(player){aimTarget=closestEnemy(player.x,player.y,520);if(aimTarget)aim=Math.atan2(aimTarget.y-player.y,aimTarget.x-player.x);else if(dx||dy)aim=Math.atan2(dy,dx);}return{dx:dx,dy:dy,angle:aim,fire:!!(keys.attack||mouse.down||buttons.fire||(mobileAimFireEnabled()&&aimJoystick.active&&aimJoystick.strength>.12)||(mobileAutoFireEnabled()&&aimTarget)),dash:!!(keys.space||buttons.dash),switch:!!(keys.q||buttons.switch),ability:!!(keys.e||buttons.ability),at:Date.now()};}
    ...
    function renderState3D(){return{scene:sceneMode,worldRevision:netWorldRevision,layoutId:currentLayout&&currentLayout.id,biome:currentBiome||BIOMES[0],zoom:currentZoom,cameraX:camera.x+viewportWidth/currentZoom/2,cameraY:camera.y+viewportHeight/currentZoom/2,player:player,players:partyPlayers(),enemies:enemies,bullets:bullets,pickups:pickups,obstacles:obstacles,decor:roomDecor,props:biomeProps,hazards:hazards,effects:effects,particles:particles,slashes:slashes,totems:totems,radiantWeapons:radiantWeapons,shopOffers:shopOffers,shopPlatforms:shopPlatforms,shopForge:shopForge,shopCraft:shopCraft,settings:runtimeSettings,tick:visualTick,palaceCutscene:palaceCutscene,palaceStatueFallen:palaceStatueFallen,drawBossWarnings:function(vector){var visuals=window.DK_BUILTIN_BOSS_VISUALS||{},api=Object.assign({},BOSS_BEHAVIOR_API,{ctx:vector});enemies.forEach(function(boss){if(!boss.dead&&boss.isBoss&&visuals[boss.bossKey]){vector.save();try{visuals[boss.bossKey](boss,api);}finally{vector.restore();}}});}};}
    function renderCurrentScene(){if(renderer3d){renderer3d.render(renderState3D());perfStats.drawnEntities=renderer3d.stats?renderer3d.stats.entities:0;}}
    ...
    window.DKGame={
        renderer3d:function(){return renderer3d;},
